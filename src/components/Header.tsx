...
const Header: React.FC = () => {
  ...
  const navigateWithTransition = (url: string) => {
    ...
    const normalizedTargetPath = url.startsWith('/') ? url.substring(1) : url
        ...
        {searchList.map((blog, index) => (
          ...
          >
            <div className="flex items-start">
              <Search className="w-4 h-4 mr-2 text-blue-400 mt-0.5 flex-shrink-0" />
              <div>
