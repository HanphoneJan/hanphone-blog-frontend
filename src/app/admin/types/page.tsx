    ...
    <div className="font-sans min-h-screen flex flex-col bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 relative overflow-hidden">
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto lg:px-2 lg:py-2 relative z-10">
        ...
        <div className="bg-slate-800/40 backdrop-blur-sm lg:rounded-xl shadow-lg border border-slate-700/50 overflow-hidden">
          ...
          <div className="py-2 px-6 border-b border-slate-700/50 grid grid-cols-[auto_1fr_auto] items-center gap-4">
            {/* 标题：固定宽度，保证完整显示 */}
            <h2 className="text-base sm:text-lg md:text-xl text-blue-100 whitespace-nowrap">
              分类列表
            </h2>
            ...
            <div className="relative min-w-[80px]">
              ...
              <input
                ...
                placeholder="搜索..."
                className="pl-10 pr-4 py-2 rounded-lg border border-slate-700 bg-slate-900/60 text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all w-full text-sm"
              />
            </div>
          ...
          {/* 分类列表区域 */}
          <div className="p-1 lg:p-3 min-h-[90vh]">
            {loading ? (
              // 加载状态骨架屏
              <div className="space-y-4 h-[400px] flex flex-col justify-center">
                {[1, 2, 3, 4, 5, 6].map(item => (
                  <div key={item} className="animate-pulse bg-slate-700/50 rounded-lg h-24"></div>
                ))}
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
                {filteredCategoryList.map((category, index) => (
                  ...
                  <div
                    key={category.id}
                    className="bg-slate-900/60 border border-slate-700 rounded-lg p-2 hover:bg-slate-800/80 transition-all duration-300 hover:shadow-md hover:border-blue-500/30 relative group h-full flex flex-col"
                  >
                    {/* 操作按钮（悬停显示） */}
                    ...
                    {/* 分类内容 */}
                    <div className="pt-2 flex flex-col items-center text-center flex-grow">
                      {/* 序号 */}
                      ...
                      {/* 分类图片 */}
                      <div className="w-24 h-24 rounded-lg overflow-hidden border border-slate-700 mb-2 flex items-center justify-center bg-slate-800/50">
                        <Image
                      ...
                      <div className="w-full mb-2 flex items-center justify-center gap-2">
                        <span className="text-[clamp(0.8rem,3vw,0.9rem)] sm:text-base text-slate-400">
                          分类名称：
                        </span>
                        <h4 className="text-blue-100 font-medium truncate text-[clamp(0.8rem,3vw,0.9rem)] sm:text-base">
                          {category.name}
                        </h4>
                      </div>
                      ...
                      <div className="w-full mt-auto flex items-center justify-center gap-2">
                        ...
                        <span className="text-green-400 font-medium flex items-center gap-1 text-[clamp(0.8rem,3vw,0.9rem)] sm:text-base">
                          {category.blogs.length}
                          <span className="text-[clamp(0.7rem,2vw,0.875rem)]">篇</span>
                        </span>
                      </div>
                    </div>
                  </div>
              ...
              <div className="h-[400px] flex flex-col items-center justify-center text-center">
                ...
                {searchKeyword ? (
                  <button
                    ...
                    className="mt-4 px-4 py-2 rounded-lg bg-slate-700 hover:bg-slate-600 text-white transition-colors text-sm"
                  >
                    清除搜索
                  </button>
                ) : (
      ...
      {isDialogOpen && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 rounded-xl border border-slate-700 w-full max-w-md p-5 sm:p-6">
            <h3 className="text-base sm:text-lg font-semibold text-blue-100 mb-4">
            ...
            <div className="space-y-4">
              {/* 分类名称输入 */}
              <div>
                <label className="block text-sm text-slate-300 mb-1">分类名称</label>
                <input
                  type="text"
                  value={formName}
              ...
              {/* 图片上传区域 */}
              <div>
                <label className="block text-sm text-slate-300 mb-2">封面图片</label>
                <div className="border-2 border-dashed border-slate-700 rounded-lg p-4 hover:border-blue-500 transition-colors relative">
                  {fileList.length > 0 ? (
                    ...
                    <>
                      <label
                        ...
                        className={`flex flex-col items-center justify-center cursor-pointer text-slate-400 hover:text-blue-400 ${
            ...
            <div className="flex justify-end gap-3 mt-6">
              <button
                ...
                className="px-3 sm:px-4 py-2 rounded-md border border-slate-700 bg-slate-800 text-slate-300 hover:bg-slate-700 transition-colors text-sm"
              >
                取消
              </button>
              <button
                ...
                className="px-3 sm:px-4 py-2 rounded-md bg-blue-600 text-white hover:bg-blue-500 transition-colors flex items-center gap-2 text-sm"
              >
      ...
      {confirmDelete !== null && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 rounded-xl border border-slate-700 w-full max-w-md p-5 sm:p-6">
            <h3 className="text-base sm:text-lg font-semibold text-blue-100 mb-2 flex items-center">
