  ...
  const renderSwitch = (checked: boolean, onChange: () => void, disabled: boolean) => (
    <div
      className={`relative w-10 h-5 rounded-full cursor-pointer transition-colors ${
        disabled ? 'bg-slate-700' : checked ? 'bg-blue-600' : 'bg-slate-600'
      }`}
    ...
    >
      ...
      >
        {checked && !disabled && (
          <Check className="absolute inset-0 m-auto w-3 h-3 text-blue-600" />
        )}
        ...
        <div className="bg-slate-800/40 backdrop-blur-sm lg:rounded-xl shadow-lg border border-slate-700/50 overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-24 py-2 rounded-lg border border-slate-700 bg-slate-900/60 text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all"
                />
                <button
                  onClick={() => getUserList()}
                  disabled={loading}
          ...
          <div className="min-h-[90vh]">
            {loading ? (
                ...
                {[1, 2, 3, 4, 5].map(item => (
                  <div key={item} className="flex animate-pulse gap-3 sm:gap-4">
                    ...
                    <div className="flex-1 grid grid-cols-4 gap-4">
                      ...
                      <div className="h-4 bg-slate-700/50 rounded w-32 self-center"></div>
                      <div className="h-4 bg-slate-700/50 rounded w-40 self-center hidden sm:block"></div>
                      <div className="h-6 bg-slate-700/50 rounded w-16 self-center"></div>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div>
                {/* 桌面端列表表头 */}
                <div className="hidden lg:grid grid-cols-12 gap-4 px-4 sm:px-6 py-4 text-sm font-medium text-slate-400 border-b border-slate-700/50">
                ...
                {/* 列表内容 */}
                <ul className="lg:divide-y lg:divide-slate-700/30">
                  {userList.map((user, index) => (
                    <li
                      key={user.id}
                      className={`px-3 sm:px-4 lg:px-6 py-3 sm:py-4 hover:bg-slate-800/60 transition-colors ${
                        index > 0 ? 'lg:border-t-0 border-t border-slate-700/30' : ''
                      }`}
                    >
                      {/* 移动端和中等屏幕布局 */}
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3 flex-1 min-w-0">
                            ...
                            <div className="min-w-0 flex-1">
                              <div className="font-medium text-blue-100 text-sm truncate">
                                {user.nickname}
                              </div>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              ...
                              className="p-1.5 rounded-full text-slate-400 hover:bg-slate-700/50 hover:text-slate-200 transition-colors"
                            >
                        ...
                        {expandedUsers.has(user.id) && (
                          <div className="mt-3 pt-3 border-t border-slate-800 space-y-2 text-sm">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-slate-400 min-w-[60px]">邮箱：</span>
                              <span className="text-slate-300 truncate">{user.email}</span>
                            </div>
                            <div className="flex items-center gap-2">
                              <Calendar className="h-4 w-4 text-slate-500 flex-shrink-0" />
                              <span className="text-slate-400 min-w-[60px]">注册：</span>
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-slate-400 min-w-[60px]">登录：</span>
                              <span className="text-slate-300 truncate">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-slate-400 min-w-[60px]">地址：</span>
                              <span className="text-slate-300 truncate">
      ...
      {confirmDelete !== null && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 border border-slate-700 rounded-lg w-full max-w-md p-6">
            ...
            <p className="text-slate-400 text-sm mb-6">确定要删除该用户吗？此操作不可撤销。</p>
