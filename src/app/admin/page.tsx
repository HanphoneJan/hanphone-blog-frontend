...
export default function DashboardPage() {
  ...
  const [stats, setStats] = useState<StatData[]>([
    { title: '总访问量', value: 0, icon: <Eye className="h-5 w-5 text-blue-400" /> },
    { title: '博客总数', value: 0, icon: <FileText className="h-5 w-5 text-blue-400" /> },
    { title: '点赞数', value: 0, icon: <ThumbsUp className="h-5 w-5 text-blue-400" /> },
    { title: '评论数', value: 0, icon: <MessageSquare className="h-5 w-5 text-blue-400" /> }
  ])
  ...
  const renderSkeleton = (index: number) => {
    ...
    return (
      <div className="space-y-3 animate-pulse h-full flex flex-col justify-center">
        <div
          className={`${iconSize} bg-slate-700/50 rounded ${
            index === selectedCard ? 'text-blue-400' : ''
          }`}
        >
          {getIcon()}
        </div>
        <div className={`h-4 bg-slate-700/50 rounded ${titleWidth}`}></div>
        <div className={`${valueHeight} bg-slate-700/50 rounded ${valueWidth} mt-auto`}></div>
      </div>
    )
  }
  ...
  return (
    <div className="font-sans min-h-screen flex flex-col bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 relative overflow-hidden">
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 py-3 relative z-10">
          ...
          {stats.map((item, index) => {
            return (
              <div
                ...
                className={`bg-slate-800/40 backdrop-blur-sm rounded-xl shadow-lg border border-slate-700/50 p-4 sm:p-5 md:p-6 cursor-pointer transition-all duration-300 hover:shadow-xl transform hover:-translate-y-1 ${
                  loading
                    ? index === selectedCard
              ...
              >
                {loading ? (
                  ...
                  <div className="h-full flex flex-col justify-between">
                    <div>{item.icon}</div>
                    <div className="mt-2">
                      <p className="text-slate-400 text-sm sm:text-base">{item.title}</p>
                      <h2 className="text-2xl sm:text-3xl font-bold mt-1 sm:mt-2 text-blue-100">
        ...
        {/* 主要图表区域 */}
        <div className="bg-slate-800/40 backdrop-blur-sm rounded-xl shadow-lg border border-slate-700/50 p-4 sm:p-6 mb-4 md:mb-6 transition-all duration-300 hover:shadow-xl">
          <div className="flex items-center mb-4 sm:mb-6">
            <BarChart3 className="h-5 w-5 mr-2 text-blue-400" />
            <h2 className="text-lg sm:text-xl font-semibold text-blue-100">网站数据统计</h2>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 mb-6 md:mb-10">
          <div className="bg-slate-800/40 backdrop-blur-sm rounded-xl shadow-lg border border-slate-700/50 p-4 sm:p-6 transition-all duration-300 hover:shadow-xl">
            <div className="flex items-center mb-4 sm:mb-6">
              <FileText className="h-5 w-5 mr-2 text-blue-400" />
              <h2 className="text-base sm:text-lg font-semibold text-blue-100">标签分布</h2>
            </div>
          ...
          <div className="bg-slate-800/40 backdrop-blur-sm rounded-xl shadow-lg border border-slate-700/50 p-4 sm:p-6 transition-all duration-300 hover:shadow-xl">
            <div className="flex items-center mb-4 sm:mb-6">
              <FileText className="h-5 w-5 mr-2 text-blue-400" />
              <h2 className="text-base sm:text-lg font-semibold text-blue-100">类型分布</h2>
            </div>
            <div className="w-full overflow-hidden" style={smallChartContainerStyle}>
          ...
          <div className="bg-slate-800/40 backdrop-blur-sm rounded-xl shadow-lg border border-slate-700/50 p-4 sm:p-6 transition-all duration-300 hover:shadow-xl">
            <div className="flex items-center mb-4 sm:mb-6">
              ...
              <h2 className="text-base sm:text-lg font-semibold text-blue-100">访问者分布</h2>
            </div>
            <div className="w-full overflow-hidden" style={smallChartContainerStyle}>
