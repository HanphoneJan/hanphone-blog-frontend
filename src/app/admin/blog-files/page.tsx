...
export default function FileManagementPage() {
  ...
  const renderMobileFileItem = (item: FileItem) => (
    <div className="border-b border-slate-800/50 p-3 last:border-0">
  ...
  return (
    <div className="font-sans min-h-screen flex flex-col bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 relative overflow-hidden">
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto lg:px-2 lg:py-2 relative z-10">
        <div className="bg-slate-800/40 backdrop-blur-sm lg:rounded-xl shadow border border-slate-700/50 overflow-hidden">
          {/* 顶部导航 - 始终保持在一行 */}
          <div className="px-4 py-3 border-b border-slate-700/50 bg-slate-900/40">
            <div className="flex items-center justify-between gap-2 min-w-0">
              <h1 className="text-lg font-bold text-slate-200 truncate">文件管理系统</h1>

              {/* 操作按钮区 - 压缩按钮大小以保持在一行 */}
              <div className="flex items-center gap-2">
                <label className="inline-flex items-center px-2.5 py-1.5 bg-blue-600 hover:bg-blue-500 text-white rounded-lg shadow transition-colors cursor-pointer text-xs">
                ...
                <button
                  ...
                  className="inline-flex items-center px-2.5 py-1.5 bg-green-600 hover:bg-green-500 text-white rounded-lg shadow transition-colors text-xs"
                  disabled={loading}
                >
          ...
          <div className="px-4 pb-1 min-h-[90vh]">
            {/* 错误提示 */}
            {error && (
              <div className="bg-red-900/20 border border-red-800/50 text-red-400 px-4 py-2 rounded-lg mb-3 flex items-center">
              ...
              <div className="flex items-center whitespace-nowrap">
                ...
                {currentDir && (
                  <button
                    ...
                    className="text-slate-400 hover:text-slate-200 flex items-center transition-colors"
                  >
            ...
            {/* 文件列表 */}
            {loading ? (
              <div className="bg-slate-900/40 rounded-lg shadow overflow-hidden border border-slate-700/50">
              ...
              <div className="bg-slate-900/40 rounded-lg shadow overflow-hidden border border-slate-700/50">
                <div className="p-4 text-center text-slate-500">当前目录为空</div>
              </div>
            ) : isMobile ? (
              // 移动设备视图 - 时间与名称水平对齐
              <div className="bg-slate-900/40 rounded-lg shadow overflow-hidden border border-slate-700/50">
                ...
                <table className="min-w-full divide-y divide-slate-700/50">
                  <thead className="bg-slate-800/60">
                    <tr>
                      ...
                      <th
                        scope="col"
                        className="px-4 py-2.5 text-right text-xs font-medium text-slate-400 uppercase tracking-wider"
                      >
                        操作
                      </th>
                    </tr>
                      ...
                      .map(item => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3 whitespace-nowrap text-right text-xs font-medium">
                            <button
                              onClick={() => openDeleteModal(item)}
                              className="p-1.25 rounded-full bg-red-600/20 text-red-400 hover:bg-red-600/30 transition-colors"
                              disabled={loading}
                              title="删除"
                            >
                              <Trash2 className="h-3.5 w-3.5" />
                      ...
                      .map(item => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3 whitespace-nowrap text-right text-xs font-medium">
                            <button
                              onClick={() => handleViewFile(item.name)}
                              className="p-1.25 rounded-full bg-blue-600/20 text-blue-400 hover:bg-blue-600/30 transition-colors mr-1.5"
                              disabled={loading}
                              title={canPreviewInBrowser(item.name) ? '查看' : '下载'}
                            >
                              <Eye className="h-3.5 w-3.5" />
                            ...
                            <button
                              onClick={() => handleDownload(item.name)}
                              className="p-1.25 rounded-full bg-green-600/20 text-green-400 hover:bg-green-600/30 transition-colors mr-1.5"
                              disabled={loading}
                              title="下载"
                            >
                              <Download className="h-3.5 w-3.5" />
      ...
      {showCreateDirModal &&
        ...
        createPortal(
          <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4 transition-opacity duration-300 animate-fadeIn">
            <div className="bg-slate-800/95 backdrop-blur-xl rounded-xl shadow-2xl border border-slate-600/30 w-full max-w-md transform transition-transform duration-300 scale-100 animate-scaleIn">
              <div className="p-4 border-b border-slate-700/50 flex justify-between items-center bg-slate-900/40 rounded-t-xl">
                <h3 className="text-base font-semibold text-slate-200 flex items-center gap-2">
                  <Folder className="h-4.5 w-4.5 text-yellow-400" />
                  创建新目录
                </h3>
                <button
                  onClick={() => {
              ...
              <div className="p-4">
                <div className="mb-3">
                  <label className="block text-xs font-medium text-slate-300 mb-1">目录名称</label>
                  <input
                    type="text"
                    value={newDirName}
                    onChange={e => setNewDirName(e.target.value)}
                    className="w-full px-3 py-2 border border-slate-700 bg-slate-900/60 text-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all text-sm"
                    placeholder="请输入目录名称"
                    autoFocus
                  />
                  {error && (
              ...
              <div className="p-3 border-t border-slate-700/50 flex justify-end gap-2 bg-slate-900/40 rounded-b-xl">
                <button
                  onClick={() => {
                    setShowCreateDirModal(false)
                    setNewDirName('')
                  ...
                  className="px-3 py-1.5 rounded-lg bg-slate-700/60 hover:bg-slate-700 text-slate-200 transition-all duration-200 text-sm"
                >
                  取消
                </button>
                <button
                  onClick={handleCreateDirectory}
                  className="px-3 py-1.5 rounded-lg transition-all duration-300 bg-blue-600 hover:bg-blue-500 text-white text-sm"
      ...
      {previewModalVisible &&
        typeof document !== 'undefined' &&
        createPortal(
          <div className="fixed inset-0 bg-black/90 backdrop-blur-sm flex items-center justify-center z-50 p-4 transition-opacity duration-300 animate-fadeIn">
            <div className="bg-slate-800/95 backdrop-blur-xl rounded-xl shadow-2xl border border-slate-600/30 w-full h-full max-w-5xl max-h-[90vh] transform transition-transform duration-300 scale-100 animate-scaleIn flex flex-col">
              <div className="p-4 border-b border-slate-700/50 flex justify-between items-center bg-slate-900/40 rounded-t-xl">
                <h3 className="text-base font-semibold text-slate-200 flex items-center gap-2">
              ...
              <div className="p-3 border-t border-slate-700/50 flex justify-end gap-2 bg-slate-900/40 rounded-b-xl">
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg bg-green-600 hover:bg-green-500 text-white transition-all duration-200 text-sm flex items-center gap-1.5"
                >
                  <Download className="h-3.5 w-3.5" />
                  下载
                </button>
      ...
      {uploading &&
        ...
        createPortal(
          <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4 transition-opacity duration-300 animate-fadeIn">
            <div className="bg-slate-800/95 backdrop-blur-xl rounded-xl shadow-2xl border border-slate-600/30 w-full max-w-md transform transition-transform duration-300 scale-100 animate-scaleIn p-4 text-center">
              <div className="animate-spin rounded-full h-10 w-10 border-t-2 border-b-2 border-blue-500 mx-auto mb-3"></div>
              <p className="text-slate-300 text-sm">文件上传中，请稍候...</p>
            </div>
          </div>,
          document.body
        )}
