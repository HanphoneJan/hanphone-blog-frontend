  ...
  return (
    <div className="font-sans min-h-screen flex flex-col bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 relative overflow-hidden">
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto lg:px-2 lg:py-2 relative z-10">
        <div className="flex flex-nowrap items-center mt-3 gap-3 mb-2">
          <div className="flex-1 min-w-0 ml-2">
            <input
              ...
              } bg-slate-900/60 text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all h-12`}
            />
          ...
          <div className="w-auto flex-shrink-0 flex gap-2">
            ...
            <button
              onClick={resetDraft}
              disabled={loading}
              className={`px-4 py-3 rounded-md transition duration-300 bg-slate-700 hover:bg-slate-600 text-white flex items-center gap-2 h-12 ${
                loading ? 'opacity-70' : ''
              }`}
            >
            ...
            {blog.id ? (
              <button
                ...
                disabled={loading}
                className={`px-4 py-3 rounded-md transition duration-300 bg-blue-600 hover:bg-blue-500 text-white flex items-center gap-1 h-12 ${
                  loading ? 'opacity-70' : ''
                }`}
              >
                {loading ? (
              ...
              <button
                ...
                disabled={loading}
                className={`px-4 py-3 rounded-md transition duration-300 bg-blue-600 hover:bg-blue-500 text-white flex items-center gap-1 mr-1 lg:mr-0 h-12 ${
                  loading ? 'opacity-70' : ''
                }`}
              >
                {loading ? (
        ...
        <div className="bg-slate-800/40 backdrop-blur-sm rounded-xl shadow border-0 border-slate-700/50  lg:p-2 mb-6 transition-all duration-300">
          <div className={`mb-1 text-sm text-slate-400`}>
            {formErrors.content && <span className="text-red-400 ml-2">{formErrors.content}</span>}
          </div>
        ...
        {publishModalVisible &&
          ...
          createPortal(
            <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4 transition-opacity duration-300 animate-fadeIn">
              <div className="bg-slate-800/95 backdrop-blur-xl rounded-xl shadow-2xl border border-slate-600/30 w-full max-w-2xl max-h-[90vh] overflow-y-auto transform transition-transform duration-300 scale-100 animate-scaleIn">
                <div className="p-6 border-b border-slate-700/50 flex justify-between items-center bg-slate-900/40 rounded-t-xl">
                ...
                <div className="p-6 space-y-6">
                  <div>
                    ...
                    <textarea
                      ...
                      } bg-slate-900/60 text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all min-h-[100px] resize-y`}
                      maxLength={200}
                    />
                    <div className="flex justify-between items-center mt-1">
                      ...
                      <p className="text-slate-500 text-sm">{blog.description.length}/200</p>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {typeList.length > 0 ? (
                          ...
                          <button
                            ...
                            className={`px-4 py-2 rounded-lg text-sm transition-all duration-200 flex items-center gap-1 ${
                              selectedTypeId === type.id
                                ...
                                : 'bg-slate-700/60 text-slate-200 hover:bg-slate-700 hover:shadow-md'
                            }`}
                          >
                        ...
                        <p className="text-slate-500 text-sm py-2 px-4 bg-slate-800/50 rounded-lg">
                          暂无分类，请请联系管理员添加
                        </p>
                      )}
                    </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2 max-h-40 overflow-y-auto p-3 bg-slate-900/40 rounded-lg border border-slate-700/50">
                      {tagList.length > 0 ? (
                        tagList.map(tag => (
                          <button
                            key={tag.id}
                  ...
                  <div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4">
                      <div
                        className="w-full sm:w-40 h-40 rounded-lg border-2 border-dashed border-slate-700 flex items-center justify-center bg-slate-900/40 cursor-pointer hover:border-blue-500 transition-all duration-300 group relative overflow-hidden"
                      ...
                      >
                        {dialogImageUrl ? (
                          ...
                          <div className="text-center group-hover:scale-105 transition-transform duration-300">
                            <Upload className="h-8 w-8 text-slate-500 mx-auto mb-2 group-hover:text-blue-400 transition-colors" />
                      ...
                      <input
                        ...
                        className="hidden"
                      />
                      <div className="flex-1 text-sm text-slate-400 bg-slate-900/30 p-4 rounded-lg">
                        <p className="font-medium text-slate-300 mb-2">图片要求：</p>
                ...
                <div className="p-6 border-t border-slate-700/50 flex justify-end gap-3 bg-slate-900/40 rounded-b-xl">
                  <button
                    onClick={() => setPublishModalVisible(false)}
                    className="px-5 py-2 rounded-lg bg-slate-700/60 hover:bg-slate-700 text-slate-200 transition-all duration-200 hover:shadow-md"
                  >
                    取消
                  </button>
                  <button
        ...
        {previewModalVisible &&
          ...
          typeof document !== 'undefined' &&
          createPortal(
            <div className="fixed inset-0 bg-black/90 backdrop-blur-lg flex items-center justify-center z-52 p-4 transition-opacity duration-300">
