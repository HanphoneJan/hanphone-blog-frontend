...
export default function TagsManagementPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 relative overflow-hidden">
      {/* 背景装饰 */}
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2">
                {filteredTagList.map(tag => (
                  <div
                    key={tag.id}
                    className="bg-slate-900/60 border border-slate-700 rounded-lg px-3 py-2 lg:p-4 hover:bg-slate-800/80 transition-all duration-300 hover:shadow-md relative group"
                  >
                    {/* 操作按钮（悬停显示） */}
                    ...
                    <div className="pt-1 space-y-1">
                      <div className="flex items-center justify-between">
                        <p className="font-medium text-blue-100 text-[clamp(0.8rem,3vw,0.9rem)] sm:text-base">
                          标签名称
                        </p>
                        <p className="text-blue-400 text-[clamp(0.8rem,3vw,0.9rem)] sm:text-base">
                          {tag.name}
                        </p>
                      </div>

                      <div className="flex items-center justify-between">
                        <p className="text-slate-300 text-[clamp(0.8rem,3vw,0.9rem)] sm:text-base">
                          关联博客
                        </p>
                        <p className="text-green-400 text-[clamp(0.8rem,3vw,0.9rem)] sm:text-base">
                          {tag.blogsNumber} 篇
                        </p>
                      </div>
                    </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 rounded-xl border border-slate-700 w-full max-w-md p-5 sm:p-6">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm text-slate-300 mb-1">标签名称</label>
                <input
                  type="text"
                  value={formName}
