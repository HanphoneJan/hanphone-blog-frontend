...
export default function Home() {
  ...
  const getBlogInfo = (blogId: number) => {
    router.push(`/blogInfo?id=${blogId}`)
    // 点击博客时关闭侧边栏
    if (screenWidth < 1024) {
      setSidebarOpen(false)
    }
  ...
  const BlogListSkeleton = () => (
    <div className="border-b border-slate-200 dark:border-slate-700/50 p-4 rounded-lg my-2">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="sm:col-span-2 flex flex-col">
          {/* 标题骨架 */}
          <div className="h-7 mb-2 bg-gradient-to-r from-slate-200 dark:from-slate-700/30 dark:via-slate-600/40 dark:to-slate-700/30 rounded w-3/4"></div>

          {/* 描述骨架 */}
          <div className="space-y-2 mb-4 flex-grow">
            <div className="h-4 bg-gradient-to-r from-slate-200 dark:from-slate-700/30 dark:via-slate-600/40 dark:to-slate-700/30 rounded "></div>
            <div className="h-4 bg-gradient-to-r from-slate-200 dark:from-slate-700/30 dark:via-slate-600/40 dark:to-slate-700/30 rounded w-5/6"></div>
            <div className="h-4 bg-gradient-to-r from-slate-200 dark:from-slate-700/30 dark:via-slate-600/40 dark:to-slate-700/30 rounded w-4/6"></div>
          </div>
  ...
  const RecommendBlogSkeleton = () => (
    <div className="flex items-center p-2 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded cursor-pointer transition-colors pl-2">
      <div className="bg-slate-200 dark:bg-slate-700 text-slate-500 dark:text-slate-300 text-xs w-5 h-5 flex items-center justify-center rounded-full mr-3">
        <div className="h-2 w-2 bg-slate-300 dark:bg-slate-600 rounded-full animate-pulse"></div>
      </div>
      <div className="h-4 bg-gradient-to-r from-slate-200 dark:from-slate-700/30 dark:via-slate-600/40 dark:to-slate-700/30 rounded animate-shimmer flex-1"></div>
    </div>
  )
                ...
                {blogListLoading ? (
                  ...
                  >
                    {blogList.map(blog => (
                      <div
                        key={blog.id}
                        className="border-b border-slate-200 dark:border-slate-700/50 p-4 hover:bg-blue-50 dark:hover:bg-blue-900/20 cursor-pointer transition-all duration-300 rounded-lg my-2"
                    ...
                    >
                      {recommendList.map((blog, index) => (
                        <div
                          key={blog.id}
                          onClick={() => getBlogInfo(blog.id)}
                          className="p-2 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded cursor-pointer transition-colors relative pl-6 text-slate-700 dark:text-slate-300"
                        >
                          <span className="absolute left-2 top-2.5 bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-xs w-4 h-4 flex items-center justify-center rounded-full">
                            {index + 1}
                          </span>
                          <span className="hover:text-blue-600 dark:hover:text-blue-300 transition-colors ml-3">
                            {blog.title}
                          </span>
                        </div>
                      ))}
